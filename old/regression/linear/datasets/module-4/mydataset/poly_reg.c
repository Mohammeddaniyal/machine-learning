...
uint8_t (*callback)(uint64_t,double,dmlfw_column_vec_double *);
};
...
uint8_t screen_logger(uint64_t iteration_number,double error_value,dmlfw_column_vec_double *predicted_values_vector)
{
	index_t r;
	double value1;
	...
	if(matrix==NULL)
	{
		matrix=dmlfw_mat_double_from_csv("data.csv",NULL,&header);
		dmlfw_mat_double_get_dimensions(matrix,&matrix_rows,&matrix_columns);
	}
	for(r=0;r<matrix_rows;++r)
	{
		value1=dmlfw_column_vec_double_get(predicted_values_vector,r);
		dmlfw_mat_double_set(matrix,r,matrix_columns-1,value1);
	}
	dmlfw_mat_double_to_csv(matrix,"tmp.csv",header);
	if(gnuplot==NULL)
	{
		gnuplot=popen("gnuplot --persist","w");
		fprintf(gnuplot,"set datafile separator ','\n");
		fprintf(gnuplot,"set datafile columnheaders\n");
		fflush(gnuplot);
	}
...
trained_parameters=dmlfw_linear_regression_gradient_descent_fit_line(args->matrix,args->target_values_vector,args->learning_rate,args->regularized_parameter,args->iteration_number,args->callback);
...
int main(int argc,char *argv[])
{
	...
	char *target;
	char *original;
	dmlfw_mat_double *original_matrix;
	dimension_t original_matrix_rows,original_matrix_columns;
	double learning_rate;
	double regularized_parameter;
	uint64_t number_of_iterations;
	char *e;
	dmlfw_row_vec_string *source_header;
	dmlfw_row_vec_string *original_header;
	index_t i;
...
printf("Specify [source csv] [test_predictions csv] [original csv] [learning rate] [regularized_parameter]  [number of iteration]\n");
	...
	//dmlfw_mat_double_get_training_testing_data(source,&training_examples_matrix,&test_examples_matrix,20);
	training_examples_matrix=dmlfw_mat_double_from_csv(source,NULL,&source_header);
	...
	dmlfw_row_vec_string_destroy(source_header);
	test_examples_matrix=dmlfw_mat_double_from_csv(source,NULL,&source_header);
...
dmlfw_mat_double_reshape(&test_examples_matrix,test_examples_matrix_rows,test_examples_matrix_columns-1);
	...
	original_matrix=dmlfw_mat_double_from_csv(original,NULL,&original_header);
	...
	dmlfw_mat_double_get_dimensions(original_matrix,&original_matrix_rows,&original_matrix_columns);
	for(i=0;i<original_matrix_rows;++i)
	{
		predicted_value=dmlfw_column_vec_double_get(test_examples_predicted_values_vector,i);
		dmlfw_mat_double_set(original_matrix,i,original_matrix_columns-1,predicted_value);
	}
	dmlfw_mat_double_to_csv(original_matrix,target,original_header);
	dmlfw_mat_double_destroy(original_matrix);
	dmlfw_row_vec_string_destroy(original_header);
	dmlfw_column_vec_double_destroy(test_examples_predicted_values_vector);
	if(gnuplot!=NULL)
	{
		pclose(gnuplot);
