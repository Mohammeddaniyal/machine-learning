...
trained_parameters=dmlfw_logistic_regression_gradient_descent_fit(args->matrix,args->target_values_vector,args->learning_rate,args->iteration_number,args->callback);
...
int main(int argc,char *argv[])
{
	...
	test_examples_predicted_values_vector=dmlfw_logistic_regression_predict(test_examples_matrix,trained_parameters);
	...
	trained_parameters_header=dmlfw_row_vec_string_create_new(trained_parameters_size);
	...
	else
	{
		for(i=0;i<trained_parameters_size;++i)
		{
			...
			dmlfw_row_vec_string_set(trained_parameters_header,i,str);
		}
		dmlfw_row_vec_double_to_csv(trained_parameters,model_csv_name,trained_parameters_header);
