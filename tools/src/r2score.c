...
void score_it(char *result_file_name)
{
	...
	dataset=dmlfw_mat_double_from_csv(result_file_name,NULL,&dataset_header);
	if(dataset==NULL)
	{
	...
	A=dmlfw_mat_double_create_column_vec(dataset,dataset_columns-2,NULL); // 2nd last column of results file
	if(A==NULL)
	{
		printf("Low memory\n");
	...
	P=dmlfw_mat_double_create_column_vec(dataset,dataset_columns-1,NULL); // last column of result file
	if(P==NULL)
	{
		printf("Low memory\n");
		dmlfw_mat_double_destroy(dataset);
	...
	M=dmlfw_column_vec_double_create_new_filled(dataset_rows,mean_of_actuals,NULL);
	if(M==NULL)
	{
		printf("Low memory\n");
		dmlfw_mat_double_destroy(dataset);
...
int main(int argc,char *argv[])
{
	if(argc!=2)
	{
		printf("Usage : [r2score result_file]\n");
		return 0;
	}
	score_it(argv[1]);
	return 0;
