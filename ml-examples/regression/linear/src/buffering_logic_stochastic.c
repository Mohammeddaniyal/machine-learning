...
void get_one_from_file_buffer(dmlfw_mat_double **target_matrix,uint64_t from_row)
{
	...
	if(from_row>buffer_matrix_1_last_possible_row)
	{
		...
		dmlfw_mat_double_copy(*target_matrix,buffer_matrix_2,0,0,from_buffer_matrix_index,0,to_buffer_matrix_index,number_of_columns_in_training_examples-1);
		return;
	}
	if(from_row>=buffer_matrix_1_starts_at_row && from_row<=buffer_matrix_1_ends_at_row)
	{
		...
		dmlfw_mat_double_copy(*target_matrix,buffer_matrix_1,0,0,from_buffer_matrix_index,0,to_buffer_matrix_index,number_of_columns_in_training_examples-1);
		return;
	}
...
int main()
{
	...
	for(y=1;y<=3;++y)
	{
		for(x=1;x<=number_of_training_examples;++x)
		{
			...
			if(dmlfw_error()) goto err;
			dmlfw_mat_double_get_dimensions(target_matrix,&rows,&columns);
			printf("%d) %lf %lf\n",x,dmlfw_mat_double_get(target_matrix,0,0),dmlfw_mat_double_get(target_matrix,0,0));
		}
		printf("Total : %d\n",x);
	}
	return 0;
