...
int on_iteration_complete(uint64_t iteration_number,void *y,void *predicted_y,void *model,double regularization_parameter)
{
	...
	static dmlfw_mat_double *actual_dataset_matrix=NULL;
	static dmlfw_row_vec_string *actual_dataset_matrix_header=NULL;
	...
	{
		actual_dataset_matrix=dmlfw_mat_double_from_csv(ACTUAL_TRAINING_DATASET,NULL,&actual_dataset_matrix_header);
		if(dmlfw_error())
		{
			printf("Unable to load actual training dataset\n");
			return -1;
		}
		dmlfw_mat_double_get_dimensions(actual_dataset_matrix,&actual_dataset_matrix_rows,&actual_dataset_matrix_columns);
	}
	...
	if(iteration_number%FREQUENCY_OF_PRINTING_COST==0 || iteration_number==NUMBER_OF_ITERATIONS || iteration_number==1)
	{
		...
		if(SHOW_GRAPH)
		{
			if(gnuplot!=NULL)
			{
				...
				for(r=0;r<y_size;++r)
				{
					value=dmlfw_column_vec_double_get(predicted_y,r);
					dmlfw_mat_double_set(actual_dataset_matrix,r,actual_dataset_matrix_columns-1,value);
				}
				dmlfw_mat_double_to_csv(actual_dataset_matrix,"curve_fit.csv",actual_dataset_matrix_header);
	...
	if(iteration_number==NUMBER_OF_ITERATIONS)
	{
		...
		dmlfw_mat_double_destroy(actual_dataset_matrix);
