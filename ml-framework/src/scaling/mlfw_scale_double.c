...
mlfw_mat_double * mlfw_scale_double_z_score_with_given_mean_standard_deviation(mlfw_mat_double *matrix,index_t start_row_index,index_t start_column_index,index_t end_row_index,index_t end_column_index,mlfw_mat_double *mean_standard_deviation_matrix,mlfw_mat_double *new_matrix)
{
	...
	{	_mlfw_set_error(MLFW_INVALID_MEAN_AND_STANDARD_DEVIATION_MATRIX_DIMENSIONS_CODE,MLFW_INVALID_MEAN_AND_STANDARD_DEVIATION_MATRIX_DIMENSIONS,"mean_standard_deviation_matrix",mean_standard_deviation_matrix_rows,mean_standard_deviation_matrix_columns,2,end_column_index-start_column_index+1);
	 	return NULL; 
	}
...
mlfw_mat_double * mlfw_scale_double(char *dataset_file_name,mlfw_row_vec_string *columns_to_scale,char *parameters_file_name,char *algorithm,mlfw_mat_double *matrix)
{
	...
	{
    _mlfw_set_error(MLFW_NULL_ARGUMENT_CODE, MLFW_NULL_ARGUMENT, "dataset_file_name");
    return NULL;
	}

	if (columns_to_scale == NULL) 	
	{
    _mlfw_set_error(MLFW_NULL_ARGUMENT_CODE, MLFW_NULL_ARGUMENT, "columns_to_scale");
    return NULL;
	}

	if (parameters_file_name == NULL) 
	{
    _mlfw_set_error(MLFW_NULL_ARGUMENT_CODE, MLFW_NULL_ARGUMENT, "parameters_file_name");
    return NULL;
	}

	if (algorithm == NULL) 
	{
    _mlfw_set_error(MLFW_NULL_ARGUMENT_CODE, MLFW_NULL_ARGUMENT, "algorithm");
    return NULL;
	}
	...
	{
		_mlfw_set_error(MLFW_INVALID_ALGORITHM_NAME_CODE,MLFW_INVALID_ALGORITHM_NAME,"algorithm");
		return NULL;
	}
	...
	{
		_mlfw_set_error(MLFW_VECTOR_EMPTY_CODE,MLFW_VECTOR_EMPTY,"columns_to_scale");
		return NULL;
	}
	...
	for(i=0;i<columns_to_scale_size;++i)
	{
		...
		for(j=0;j<matrix_header_size;++j)
		{
			...
			if(algorithm_code==MIN_MAX_ALGORITHM)
			{
			mlfw_scale_double_min_max(matrix,0,j,matrix_rows-1,j,&tmp_matrix,scaled_column_matrix);
			if(mlfw_error())
			{
				mlfw_mat_double_destroy(parameters_matrix);
				mlfw_mat_double_destroy(matrix);
