#include<stdio.h>
#include<dmlfw_algorithms.h>
#include<dmlfw_vector.h>
#include<dmlfw_matrix.h>
#include<dmlfw_operations.h>
#include<___dmlfw_error.h>
#include<dmlfw_error.h>
#include<stdlib.h>
extern __thread uint32_t _dmlfw_error_code;
extern __thread char _dmlfw_error_string[512];
extern __thread char _dmlfw_debug_string[512];

dmlfw_column_vec_double * dmlfw_linear_regression_fit_using_batch_gradient_descent(dmlfw_gradient_descent_options  *gd_options,dmlfw_mat_double *x,dmlfw_column_vec_double *y,double regularization_parameter,dmlfw_column_vec_double *model)
{
	dimension_t x_rows,x_columns;
	dimension_t theta_rows;
	double theta_0; // to store value of theta[0] before setting to 0
				 // as it is to be ignore in regularization term
	dmlfw_gradient_descent_lin_reg_progress_callback_t progress_callback;
	
	dmlfw_mat_double *x_transposed=NULL;
	dmlfw_column_vec_double *predicted_y=NULL;
	dmlfw_column_vec_double *prediction_error=NULL;
	dmlfw_column_vec_double *theta=NULL;
	dmlfw_column_vec_double *tmp1=NULL;
	dmlfw_column_vec_double *tmp2=NULL;
	dmlfw_column_vec_double *tmp3=NULL;
	dmlfw_column_vec_double *tmp4=NULL;
	dmlfw_column_vec_double *regularization_term=NULL;
	dmlfw_column_vec_double *tmp_theta=NULL;

	index_t n;
	uint64_t number_of_iterations;
	double learning_rate;
	dimension_t model_size;
	dmlfw_reset_error();
	// validations
	x_transposed=dmlfw_mat_double_transpose(x,NULL);
	if(dmlfw_error()) goto err;
	// setup initial parameters
	dmlfw_mat_double_get_dimensions(x,&x_rows,&x_columns);
	theta=dmlfw_column_vec_double_create_new_filled(x_columns,0.0,NULL);
	if(dmlfw_error()) goto err;

	if(model!=NULL)
	{	
		// validate model size
		model_size=dmlfw_column_vec_double_get_size(model);
		if(model_size!=x_columns)
		{
			_dmlfw_set_error(MLFW_INVALID_MODEL_VECTOR_SIZE_CODE,MLFW_INVALID_MODEL_VECTOR_SIZE,"model",model_size,x_columns);
			goto err;
		}
		// populate theta with values from model
		dmlfw_column_vec_double_copy(theta,model);
		if(dmlfw_error()) goto err;
	}
	// created vector for storing predicted values
	theta_rows=x_columns;
	predicted_y=dmlfw_column_vec_double_create_new(x_rows);
	if(dmlfw_error()) goto err;
	prediction_error=dmlfw_column_vec_double_create_new(x_rows);
	if(dmlfw_error()) goto err;
	tmp1=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;
	tmp2=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;
	tmp3=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;
	tmp4=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;
	tmp_theta=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;
	regularization_term=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;

	// i can also apply validation here
	number_of_iterations=dmlfw_gradient_descent_options_get_number_of_iterations(gd_options);
	learning_rate=dmlfw_gradient_descent_options_get_learning_rate(gd_options);
	progress_callback=dmlfw_gradient_descent_options_get_progress_callback(gd_options);
	n=1;
	while(n<=number_of_iterations)
	{
		// predict
		dmlfw_multiply_double_matrix_with_column_vector(x,theta,predicted_y);
		if(dmlfw_error()) goto err;
		// compute error
		dmlfw_subtract_double_column_vector(predicted_y,y,prediction_error);
		if(dmlfw_error()) goto err;
		// tmp calculation, store in tmp1=x_transposed * prediction_error
		dmlfw_multiply_double_matrix_with_column_vector(x_transposed,prediction_error,tmp1);
		if(dmlfw_error()) goto err;
		// tmp calculation, store in tmp2=(1.0/x_rows))*tmp2;
		dmlfw_multiply_double_scalar_with_column_vector(1.0/x_rows,tmp1,tmp2);
		if(dmlfw_error()) goto err;
		// pick theta[0]
		theta_0=dmlfw_column_vec_double_get(theta,0);
		if(dmlfw_error()) goto err;
		// set theta[0] for regularization
		dmlfw_column_vec_double_set(theta,0,0.0);
		if(dmlfw_error()) goto err;
		// regularization_term calculation, store in regularization_term=lambda/m*theta (ignoring_bias)
		dmlfw_multiply_double_scalar_with_column_vector(regularization_parameter/x_rows,theta,regularization_term);
		if(dmlfw_error()) goto err;
		// set back theta[0], what we picked into theta_0
		dmlfw_column_vec_double_set(theta,0,theta_0);
		if(dmlfw_error()) goto err;
		// tmp calculation, tmp3=tmp2+regularization_term
		dmlfw_add_double_column_vector(tmp2,regularization_term,tmp3);
		if(dmlfw_error()) goto err;
		// tmp calculation, tmp4=learning_rate*tmp3
		dmlfw_multiply_double_scalar_with_column_vector(learning_rate,tmp3,tmp4);
		if(dmlfw_error()) goto err;
		// tmp theta calculation, store in tmp_theta=theta-tmp4
		dmlfw_subtract_double_column_vector(theta,tmp4,tmp_theta);
		if(dmlfw_error()) goto err;
		// copy tmp_theta to theta, to update theta
		dmlfw_column_vec_double_copy(theta,tmp_theta);
		if(dmlfw_error()) goto err;
		// theta updated
		// iteration complete, call progress_callback, if it returns, -1 break
		if(progress_callback!=NULL)
		{
		if(progress_callback(n,y,predicted_y,theta,regularization_parameter) == -1) break;
		}
		++n;
		}
		dmlfw_mat_double_destroy(x_transposed);
		dmlfw_column_vec_double_destroy(predicted_y);
		dmlfw_column_vec_double_destroy(prediction_error);
		dmlfw_column_vec_double_destroy(tmp1);
		dmlfw_column_vec_double_destroy(tmp2);
		dmlfw_column_vec_double_destroy(tmp3);
		dmlfw_column_vec_double_destroy(tmp4);
		dmlfw_column_vec_double_destroy(tmp_theta);
		dmlfw_column_vec_double_destroy(regularization_term);
		return theta;
		err:
		dmlfw_mat_double_destroy(x_transposed);
		dmlfw_column_vec_double_destroy(predicted_y);
		dmlfw_column_vec_double_destroy(prediction_error);
		dmlfw_column_vec_double_destroy(theta);
		dmlfw_column_vec_double_destroy(theta);
		dmlfw_column_vec_double_destroy(tmp1);
		dmlfw_column_vec_double_destroy(tmp2);
		dmlfw_column_vec_double_destroy(tmp3);
		dmlfw_column_vec_double_destroy(tmp4);
		dmlfw_column_vec_double_destroy(tmp_theta);
		dmlfw_column_vec_double_destroy(regularization_term);
		return NULL;
}
dmlfw_column_vec_double * dmlfw_linear_regression_predict(dmlfw_mat_double *x,dmlfw_column_vec_double *model)
{
	dmlfw_column_vec_double *predicted_y;
	dmlfw_reset_error();
	predicted_y=dmlfw_multiply_double_matrix_with_column_vector(x,model,NULL);
	if(dmlfw_error()) return NULL;
	return predicted_y;
}

dmlfw_column_vec_double * dmlfw_linear_regression_fit_using_stochastic_gradient_descent(dmlfw_gradient_descent_options *gd_options,double regularization_parameter,dmlfw_column_vec_double *model)
{
	int end_process=0;
	uint64_t from_row_number;
	uint32_t how_many_rows;
	dimension_t x_rows,x_columns;
	dimension_t theta_rows;
	double theta_0; // to store value of theta[0] before setting to 0
				 // as it is to be ignore in regularization term
	dmlfw_gradient_descent_lin_reg_progress_callback_t progress_callback;

	dmlfw_mat_double *x=NULL;
	dmlfw_column_vec_double *y=NULL;	
	dmlfw_mat_double *x_transposed=NULL;
	dmlfw_column_vec_double *predicted_y=NULL;
	dmlfw_column_vec_double *prediction_error=NULL;
	dmlfw_column_vec_double *theta=NULL;
	dmlfw_column_vec_double *tmp1=NULL;
	dmlfw_column_vec_double *tmp2=NULL;
	dmlfw_column_vec_double *tmp3=NULL;
	dmlfw_column_vec_double *tmp4=NULL;
	dmlfw_column_vec_double *regularization_term=NULL;
	dmlfw_column_vec_double *tmp_theta=NULL;

	index_t n;
	uint64_t number_of_iterations;
	double learning_rate;
	dimension_t model_size;
	dmlfw_gradient_descent_lin_reg_data_provider_t data_provider;
	dmlfw_reset_error();
	// validations
	// i can also apply validation here
	number_of_iterations=dmlfw_gradient_descent_options_get_number_of_iterations(gd_options);
	learning_rate=dmlfw_gradient_descent_options_get_learning_rate(gd_options);
	progress_callback=dmlfw_gradient_descent_options_get_progress_callback(gd_options);
	data_provider=dmlfw_gradient_descent_options_get_data_provider(gd_options);
	if(data_provider==NULL)
	{
		_dmlfw_set_error(MLFW_NULL_ARGUMENT_CODE,MLFW_NULL_ARGUMENT,"gd_options[data_provider]");
		goto err;
	}

	// get first data, so that we can perform initial setup
	how_many_rows=1;
	data_provider(&x,&y,1,how_many_rows);
	if(x==NULL)
	{
		_dmlfw_set_error(MLFW_DATA_PROVIDER_PROVIDED_NO_ROWS_CODE,MLFW_DATA_PROVIDER_PROVIDED_NO_ROWS);
		goto err;
	}
	// set up initial parameters
	dmlfw_mat_double_get_dimensions(x,&x_rows,&x_columns);
	theta=dmlfw_column_vec_double_create_new_filled(x_columns,0.0,NULL);
	if(dmlfw_error()) goto err;
	x_transposed=dmlfw_mat_double_transpose(x,NULL);
	if(dmlfw_error()) goto err;

	if(model!=NULL)
	{	
		// validate model size
		model_size=dmlfw_column_vec_double_get_size(model);
		if(model_size!=x_columns)
		{
			_dmlfw_set_error(MLFW_INVALID_MODEL_VECTOR_SIZE_CODE,MLFW_INVALID_MODEL_VECTOR_SIZE,"model",model_size,x_columns);
			goto err;
		}
		// populate theta with values from model
		dmlfw_column_vec_double_copy(theta,model);
		if(dmlfw_error()) goto err;
	}
	// created vector for storing predicted values
	theta_rows=x_columns;
	predicted_y=dmlfw_column_vec_double_create_new(x_rows);
	if(dmlfw_error()) goto err;
	prediction_error=dmlfw_column_vec_double_create_new(x_rows);
	if(dmlfw_error()) goto err;
	tmp1=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;
	tmp2=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;
	tmp3=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;
	tmp4=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;
	tmp_theta=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;
	regularization_term=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;

	n=1;
	while(1)
	{
	if(n>number_of_iterations) break;
	from_row_number=1; // we are fetching 1, earlier to just for initial setup
	while(1)
	{
		data_provider(&x,&y,from_row_number,how_many_rows);
		if(dmlfw_error())
		{
			goto err;
		}
		if(x==NULL)
		{
			break;
		}
			//transpose x
				dmlfw_mat_double_transpose(x,x_transposed);
				if(dmlfw_error()) goto err;
		// predict
		dmlfw_multiply_double_matrix_with_column_vector(x,theta,predicted_y);
		if(dmlfw_error()) goto err;
		// compute error
		dmlfw_subtract_double_column_vector(predicted_y,y,prediction_error);
		if(dmlfw_error()) goto err;
		// tmp calculation, store in tmp1=x_transposed * prediction_error
		dmlfw_multiply_double_matrix_with_column_vector(x_transposed,prediction_error,tmp1);
		if(dmlfw_error()) goto err;
		// tmp calculation, store in tmp2=(1.0/x_rows))*tmp2;
		dmlfw_multiply_double_scalar_with_column_vector(1.0/x_rows,tmp1,tmp2);
		if(dmlfw_error()) goto err;
		// pick theta[0]
		theta_0=dmlfw_column_vec_double_get(theta,0);
		if(dmlfw_error()) goto err;
		// set theta[0] for regularization
		dmlfw_column_vec_double_set(theta,0,0.0);
		if(dmlfw_error()) goto err;
		// regularization_term calculation, store in regularization_term=lambda/m*theta (ignoring_bias)
		dmlfw_multiply_double_scalar_with_column_vector(regularization_parameter/x_rows,theta,regularization_term);
		if(dmlfw_error()) goto err;
		// set back theta[0], what we picked into theta_0
		dmlfw_column_vec_double_set(theta,0,theta_0);
		if(dmlfw_error()) goto err;
		// tmp calculation, tmp3=tmp2+regularization_term
		dmlfw_add_double_column_vector(tmp2,regularization_term,tmp3);
		if(dmlfw_error()) goto err;
		// tmp calculation, tmp4=learning_rate*tmp3
		dmlfw_multiply_double_scalar_with_column_vector(learning_rate,tmp3,tmp4);
		if(dmlfw_error()) goto err;
		// tmp theta calculation, store in tmp_theta=theta-tmp4
		dmlfw_subtract_double_column_vector(theta,tmp4,tmp_theta);
		if(dmlfw_error()) goto err;
		// copy tmp_theta to theta, to update theta
		dmlfw_column_vec_double_copy(theta,tmp_theta);
		if(dmlfw_error()) goto err;
		// theta updated
		// iteration complete, call progress_callback, if it returns, -1 break
		if(progress_callback!=NULL)
		{
		end_process=progress_callback(n,y,predicted_y,theta,regularization_parameter);
		}
		from_row_number++;
		}// infinite loop ends
		if(end_process==-1) break;
		++n;
		}
		dmlfw_mat_double_destroy(x_transposed);
		dmlfw_column_vec_double_destroy(predicted_y);
		dmlfw_column_vec_double_destroy(prediction_error);
		dmlfw_column_vec_double_destroy(tmp1);
		dmlfw_column_vec_double_destroy(tmp2);
		dmlfw_column_vec_double_destroy(tmp3);
		dmlfw_column_vec_double_destroy(tmp4);
		dmlfw_column_vec_double_destroy(tmp_theta);
		dmlfw_column_vec_double_destroy(regularization_term);
		return theta;
		err:
		dmlfw_mat_double_destroy(x_transposed);
		dmlfw_column_vec_double_destroy(predicted_y);
		dmlfw_column_vec_double_destroy(prediction_error);
		dmlfw_column_vec_double_destroy(theta);
		dmlfw_column_vec_double_destroy(tmp1);
		dmlfw_column_vec_double_destroy(tmp2);
		dmlfw_column_vec_double_destroy(tmp3);
		dmlfw_column_vec_double_destroy(tmp4);
		dmlfw_column_vec_double_destroy(tmp_theta);
		dmlfw_column_vec_double_destroy(regularization_term);
		return NULL; 
}

dmlfw_column_vec_double * dmlfw_linear_regression_fit_using_mini_batch_gradient_descent(dmlfw_gradient_descent_options *gd_options,double regularization_parameter,dmlfw_column_vec_double *model)
{
	int end_process=0;
	uint64_t from_row_number;
	uint32_t how_many_rows;
	dimension_t x_rows,x_columns;
	dimension_t theta_rows;
	double theta_0; // to store value of theta[0] before setting to 0
				 // as it is to be ignore in regularization term
	dmlfw_gradient_descent_lin_reg_progress_callback_t progress_callback;

	dmlfw_mat_double *x=NULL;
	dmlfw_column_vec_double *y=NULL;	
	dmlfw_mat_double *x_transposed=NULL;
	dmlfw_column_vec_double *predicted_y=NULL;
	dmlfw_column_vec_double *prediction_error=NULL;
	dmlfw_column_vec_double *theta=NULL;
	dmlfw_column_vec_double *tmp1=NULL;
	dmlfw_column_vec_double *tmp2=NULL;
	dmlfw_column_vec_double *tmp3=NULL;
	dmlfw_column_vec_double *tmp4=NULL;
	dmlfw_column_vec_double *regularization_term=NULL;
	dmlfw_column_vec_double *tmp_theta=NULL;

	index_t n;
	uint64_t number_of_iterations;
	double learning_rate;
	dimension_t model_size;
	dmlfw_gradient_descent_lin_reg_data_provider_t data_provider;
	uint32_t mini_batch_size;
	dmlfw_reset_error();
	// validations
	// i can also apply validation here
	number_of_iterations=dmlfw_gradient_descent_options_get_number_of_iterations(gd_options);
	learning_rate=dmlfw_gradient_descent_options_get_learning_rate(gd_options);
	progress_callback=dmlfw_gradient_descent_options_get_progress_callback(gd_options);
	data_provider=dmlfw_gradient_descent_options_get_data_provider(gd_options);
	mini_batch_size=dmlfw_gradient_descent_options_get_mini_batch_size(gd_options);
	if(data_provider==NULL)
	{
		_dmlfw_set_error(MLFW_NULL_ARGUMENT_CODE,MLFW_NULL_ARGUMENT,"gd_options[data_provider]");
		goto err;
	}

	// get first data, so that we can perform initial setup
	how_many_rows=mini_batch_size;
	data_provider(&x,&y,1,how_many_rows);
	if(x==NULL)
	{
		_dmlfw_set_error(MLFW_DATA_PROVIDER_PROVIDED_NO_ROWS_CODE,MLFW_DATA_PROVIDER_PROVIDED_NO_ROWS);
		goto err;
	}
	// set up initial parameters
	dmlfw_mat_double_get_dimensions(x,&x_rows,&x_columns);
	theta=dmlfw_column_vec_double_create_new_filled(x_columns,0.0,NULL);
	if(dmlfw_error()) goto err;
	x_transposed=dmlfw_mat_double_transpose(x,NULL);
	if(dmlfw_error()) goto err;

	if(model!=NULL)
	{	
		// validate model size
		model_size=dmlfw_column_vec_double_get_size(model);
		if(model_size!=x_columns)
		{
			_dmlfw_set_error(MLFW_INVALID_MODEL_VECTOR_SIZE_CODE,MLFW_INVALID_MODEL_VECTOR_SIZE,"model",model_size,x_columns);
			goto err;
		}
		// populate theta with values from model
		dmlfw_column_vec_double_copy(theta,model);
		if(dmlfw_error()) goto err;
	}
	// created vector for storing predicted values
	theta_rows=x_columns;
	predicted_y=dmlfw_column_vec_double_create_new(x_rows);
	if(dmlfw_error()) goto err;
	prediction_error=dmlfw_column_vec_double_create_new(x_rows);
	if(dmlfw_error()) goto err;
	tmp1=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;
	tmp2=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;
	tmp3=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;
	tmp4=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;
	tmp_theta=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;
	regularization_term=dmlfw_column_vec_double_create_new(theta_rows);
	if(dmlfw_error()) goto err;
	
	n=1;
	while(1)
	{
	if(n>number_of_iterations) break;
	from_row_number=1; // we are fetching 1, earlier to just for initial setup
	while(1)
	{
		data_provider(&x,&y,from_row_number,how_many_rows);
		if(dmlfw_error())
		{
			goto err;
		}
		if(x==NULL)
		{
			break;
		}
			//transpose x
				dmlfw_mat_double_transpose(x,x_transposed);
				if(dmlfw_error()) goto err;
		// predict
		dmlfw_multiply_double_matrix_with_column_vector(x,theta,predicted_y);
		if(dmlfw_error()) goto err;
		// compute error
		dmlfw_subtract_double_column_vector(predicted_y,y,prediction_error);
		if(dmlfw_error()) goto err;
		// tmp calculation, store in tmp1=x_transposed * prediction_error
		dmlfw_multiply_double_matrix_with_column_vector(x_transposed,prediction_error,tmp1);
		if(dmlfw_error()) goto err;
		// tmp calculation, store in tmp2=(1.0/x_rows))*tmp2;
		dmlfw_multiply_double_scalar_with_column_vector(1.0/x_rows,tmp1,tmp2);
		if(dmlfw_error()) goto err;
		// pick theta[0]
		theta_0=dmlfw_column_vec_double_get(theta,0);
		if(dmlfw_error()) goto err;
		// set theta[0] for regularization
		dmlfw_column_vec_double_set(theta,0,0.0);
		if(dmlfw_error()) goto err;
		// regularization_term calculation, store in regularization_term=lambda/m*theta (ignoring_bias)
		dmlfw_multiply_double_scalar_with_column_vector(regularization_parameter/x_rows,theta,regularization_term);
		if(dmlfw_error()) goto err;
		// set back theta[0], what we picked into theta_0
		dmlfw_column_vec_double_set(theta,0,theta_0);
		if(dmlfw_error()) goto err;
		// tmp calculation, tmp3=tmp2+regularization_term
		dmlfw_add_double_column_vector(tmp2,regularization_term,tmp3);
		if(dmlfw_error()) goto err;
		// tmp calculation, tmp4=learning_rate*tmp3
		dmlfw_multiply_double_scalar_with_column_vector(learning_rate,tmp3,tmp4);
		if(dmlfw_error()) goto err;
		// tmp theta calculation, store in tmp_theta=theta-tmp4
		dmlfw_subtract_double_column_vector(theta,tmp4,tmp_theta);
		if(dmlfw_error()) goto err;
		// copy tmp_theta to theta, to update theta
		dmlfw_column_vec_double_copy(theta,tmp_theta);
		if(dmlfw_error()) goto err;
		// theta updated
		// iteration complete, call progress_callback, if it returns, -1 break
		if(progress_callback!=NULL)
		{
		end_process=progress_callback(n,y,predicted_y,theta,regularization_parameter);
		}
		from_row_number++;
		}// infinite loop ends
		if(end_process==-1) break;
		++n;
		}
		dmlfw_mat_double_destroy(x_transposed);
		dmlfw_column_vec_double_destroy(predicted_y);
		dmlfw_column_vec_double_destroy(prediction_error);
		dmlfw_column_vec_double_destroy(tmp1);
		dmlfw_column_vec_double_destroy(tmp2);
		dmlfw_column_vec_double_destroy(tmp3);
		dmlfw_column_vec_double_destroy(tmp4);
		dmlfw_column_vec_double_destroy(tmp_theta);
		dmlfw_column_vec_double_destroy(regularization_term);
		return theta;
		err:
		dmlfw_mat_double_destroy(x_transposed);
		dmlfw_column_vec_double_destroy(predicted_y);
		dmlfw_column_vec_double_destroy(prediction_error);
		dmlfw_column_vec_double_destroy(theta);
		dmlfw_column_vec_double_destroy(tmp1);
		dmlfw_column_vec_double_destroy(tmp2);
		dmlfw_column_vec_double_destroy(tmp3);
		dmlfw_column_vec_double_destroy(tmp4);
		dmlfw_column_vec_double_destroy(tmp_theta);
		dmlfw_column_vec_double_destroy(regularization_term);
		return NULL; 
}

